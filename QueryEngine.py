...
class Postgresql(DbConnectionSkelton):
    ...
    def connect(self):
        conn = psycopg2.connect(
              host = self.coninfo['host']
             ,port = self.coninfo['port']
             ,user = self.coninfo['user']
             ,password = self.coninfo['user']
             ,database = self.coninfo['database'] 
        )
...
class Presto(DbConnectionSkelton):
    ...
    def connect(self):
        conn = prestodb.connect(
              host = self.coninfo['host']
             ,port = self.coninfo['port']
             ,user = self.coninfo['user'] ,password = self.coninfo['user']
             ,database = self.coninfo['database'] 
        )
